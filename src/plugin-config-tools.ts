      ...
      try {
        const command = `
          ...
          if command -v python3 &> /dev/null; then
            python3 -c "import yaml, sys; yaml.safe_load(open('$compose_file'))" 2>&1
            if [ $? -eq 0 ]; then
          ...
          if grep -q "^networks:" "$compose_file" || grep -q "^  networks:" "$compose_file"; then
            echo "✓ 'networks' section found"
          else
            echo "⚠ 'networks' section not found (optional)"
          fi

          if grep -q "^volumes:" "$compose_file" || grep -q "^  volumes:" "$compose_file"; then
            echo "✓ 'volumes' section found"
          else
            echo "⚠ 'volumes' section not found (optional)"
          fi

          # Check version if specified
          if grep -q "^version:" "$compose_file"; then
            version=$(grep "^version:" "$compose_file" | cut -d: -f2 | tr -d " '\"")
            echo "ℹ Compose file version: $version"
          fi
        `.trim();
        ...
        return {
          content: [
            {
              type: "text",
              text: `Failed to validate Docker Compose file: ${error instanceof Error ? error.message : String(error)}`,
            },
          ],
          isError: true,
        };
